...
class CA(object):
    ...
    def norm_plot(self):
        """Plot the first and second dimensions after normalization."""
        # parties = dict()
        # with open('sen_account_ids.csv', 'r', encoding='utf-8', newline='') as f:
        #     reader = csv.DictReader(f)
        #     for line in reader:
        #         parties[line['name']] = line['party']

        xmin, xmax = None, None
        ymin, ymax = None, None
        if self.rows is not None:
            for i, t in enumerate(self.rows):
                x, y = self.norm_G[i,0], self.norm_G[i,1]
                # if parties[t] == "R":
                #     clr = 'r'
                # else:
                #     clr = 'b'
...
def main():
    ...
    df = pd.io.parsers.read_csv('frequency_matrix.csv')
